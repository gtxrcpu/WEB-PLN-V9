...
<head>
    <meta charset="utf-8">
    <title>Kartu Pemeriksaan APAR — {{ $apar->serial_no }}</title>
...
<div class="no-print bg-white border-b shadow-sm sticky top-0 z-10">
    <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
        <div>
            <h1 class="text-lg font-bold text-gray-900">Kartu Pemeriksaan APAR</h1>
            <p class="text-sm text-gray-500">{{ $apar->serial_no }}
                @if($apar->location_code) — {{ $apar->location_code }} @endif
            </p>
        </div>
...
<div class="max-w-5xl mx-auto px-4 py-6">
    <div class="sheet-a4 bg-white rounded-xl shadow-lg border p-8">
        ...
        <div class="flex items-start justify-between mb-6 pb-4 border-b-2 border-gray-200">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">KARTU PEMERIKSAAN APAR</h2>
        ...
        {{-- ── FORM ── --}}
        <form method="POST" action="{{ route('apar.kartu-pemeriksaan.store') }}" id="pemeriksaanForm">
            @csrf
            <input type="hidden" name="apar_id" value="{{ $apar->id }}">

            {{-- ── TABEL PEMERIKSAAN sesuai format standar ── --}}
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="border border-gray-300 px-1 py-0.5">
                                    <input type="text"
                                           ...
                                           class="w-full px-1.5 py-1 text-xs border-0 focus:ring-1 focus:ring-emerald-400 rounded bg-transparent placeholder-gray-300">
                                </td>
                                ...
                                <td class="border border-gray-300 px-1 py-0.5">
                                    <input type="text"
                                           ...
                                           placeholder="No. Seri"
                                           class="w-full px-1.5 py-1 text-xs border-0 focus:ring-1 focus:ring-emerald-400 rounded bg-transparent placeholder-gray-300 font-mono">
                                </td>

                                {{-- JENIS KIMIA --}}
                                <td class="border border-gray-300 px-1 py-0.5">
                                    <input type="text"
                                           name="rows[{{ $i }}][jenis_kimia]"
                                           value="{{ old("rows.{$i}.jenis_kimia") }}"
                                           placeholder="CO2 / Dry Powder"
                                           class="w-full px-1.5 py-1 text-xs border-0 focus:ring-1 focus:ring-emerald-400 rounded bg-transparent placeholder-gray-300">
                                </td>
                                ...
                                <td class="border border-gray-300 px-1 py-0.5">
                                    <input type="text"
                                           name="rows[{{ $i }}][berat]"
                                           value="{{ old("rows.{$i}.berat") }}"
                                           placeholder="kg"
                                           class="w-full px-1.5 py-1 text-xs border-0 focus:ring-1 focus:ring-emerald-400 rounded bg-transparent placeholder-gray-300 text-center">
                                </td>

                                {{-- KONDISI --}}
                                <td class="border border-gray-300 px-1 py-0.5">
                                    <select name="rows[{{ $i }}][kondisi]"
                                            class="w-full px-1 py-1 text-xs border border-gray-200 rounded focus:ring-1 focus:ring-emerald-400 bg-white">
                                        <option value="">—</option>
                                        ...
                                        <option value="Isi Ulang"  {{ old("rows.{$i}.kondisi") === 'Isi Ulang'  ? 'selected' : '' }}>Isi Ulang</option>
                                ...
                                <td class="border border-gray-300 px-1 py-0.5">
                                    <input type="date"
                                           name="rows[{{ $i }}][tgl_pengisian]"
                                           value="{{ old("rows.{$i}.tgl_pengisian") }}"
                                           class="w-full px-1 py-1 text-xs border-0 focus:ring-1 focus:ring-emerald-400 rounded bg-transparent">
                                </td>

                                {{-- TGL KADALUARSA --}}
                                <td class="border border-gray-300 px-1 py-0.5">
                                    <input type="date"
                                           name="rows[{{ $i }}][tgl_kadaluarsa]"
                                           value="{{ old("rows.{$i}.tgl_kadaluarsa") }}"
                                           class="w-full px-1 py-1 text-xs border-0 focus:ring-1 focus:ring-emerald-400 rounded bg-transparent">
                                </td>
            ...
            <div class="no-print mt-8 pt-6 border-t border-gray-200 flex items-center justify-between gap-3">
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit" id="submitBtn"
                            class="px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-sm font-semibold shadow-md transition-colors">
