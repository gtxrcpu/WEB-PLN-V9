...
<head>
    <meta charset="utf-8">
    <title>Kartu Kendali APAR - {{ $apar->barcode ?? $apar->serial_no }}</title>
...
<div class="no-print bg-white border-b shadow-sm sticky top-0 z-10">
    <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
        <div>
            @php $isPmk = $kartu->catatan && str_starts_with($kartu->catatan, '[PMK]'); @endphp
            <h1 class="text-lg font-bold text-gray-900">{{ $isPmk ? 'Kartu Pemeriksaan APAR' : 'Kartu Kendali APAR' }}</h1>
            <p class="text-sm text-gray-600">{{ $apar->barcode ?? $apar->serial_no }}</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('apar.riwayat', $apar->id) }}" class="px-4 py-2 border rounded-lg hover:bg-gray-50 text-sm font-medium">
                ← Kembali
            </a>
...
<div class="no-print max-w-5xl mx-auto px-4 pt-4">
    <div class="p-4 rounded-lg border
        ...
        @elseif($kartu->rejected_at || $kartu->leader_rejected_at) bg-red-50 border-red-200
        ...
        <div class="flex items-center gap-3">
            @if($kartu->isApproved())
                ...
                <div>
                    ...
                    <p class="text-sm text-green-700">
                        Disetujui oleh {{ get_user_display_name($kartu->approver ?? $kartu->leaderApprover, 'User') }}
                        pada {{ ($kartu->approved_at ?? $kartu->leader_approved_at)->format('d M Y, H:i') }} WIB
                    </p>
                </div>
            @elseif($kartu->rejected_at || $kartu->leader_rejected_at)
                ...
                <div>
                    ...
                    @if($kartu->rejection_reason || $kartu->leader_rejection_reason)
                        <p class="text-sm text-red-700 mt-1">Alasan: {{ $kartu->rejection_reason ?? $kartu->leader_rejection_reason }}</p>
                    @endif
                </div>
            @elseif($kartu->leader_approved_at)
                ...
                <div>
                    <p class="font-semibold text-yellow-800">Menunggu Approval Admin</p>
                    <p class="text-sm text-yellow-700">Sudah disetujui leader, menunggu approval admin/superadmin</p>
                </div>
            @else
                <svg class="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    <p class="font-semibold text-yellow-800">Menunggu Approval Leader</p>
...
<div class="max-w-5xl mx-auto px-4 py-6">
    <div class="sheet-a4 bg-white rounded-xl shadow-lg border p-8">
        ...
        @php
            // Deteksi apakah ini kartu pemeriksaan (prefix [PMK]) atau kartu kendali
            $isPemeriksaan = $kartu->catatan && str_starts_with($kartu->catatan, '[PMK]');
        @endphp

        {{-- HEADER KARTU - FROM TEMPLATE (sama persis dengan create) --}}
        @if($template)
            ...
            <table class="w-full text-sm">
                <tr>
                    <td rowspan="{{ count($template->header_fields) }}" class="border-r-2 border-gray-800 p-4 text-center align-middle w-2/3">
                        @if($isPemeriksaan)
                            <div class="font-bold text-2xl">KARTU PEMERIKSAAN APAR</div>
                            <div class="font-semibold text-base mt-1">TAHUN {{ \Carbon\Carbon::parse($kartu->tgl_periksa)->format('Y') }}</div>
                        @else
        ...
        {{-- INFO APAR (hanya untuk kartu kendali, format tabel formal) --}}
        @if(!$isPemeriksaan)
        <table class="w-full text-sm border-collapse border border-gray-800 mb-0">
            <tr>
                ...
                <td class="border border-gray-800 px-3 py-2">: {{ $apar->location_code ?? '-' }}</td>
                <td class="border border-gray-800 px-3 py-2 font-bold w-28">ISI APAR</td>
                <td class="border border-gray-800 px-3 py-2">: {{ $apar->type ?? '-' }}</td>
            </tr>
            <tr>
                <td class="border border-gray-800 px-3 py-2 font-bold">KAPASITAS</td>
                <td class="border border-gray-800 px-3 py-2">: {{ $apar->capacity ?? '-' }}</td>
                <td class="border border-gray-800 px-3 py-2 font-bold">MASA BERLAKU</td>
                <td class="border border-gray-800 px-3 py-2">: {{ $apar->expiry_date ? \Carbon\Carbon::parse($apar->expiry_date)->format('d F Y') : '-' }}</td>
            </tr>
        </table>
        @endif
        ...
        @if($isPemeriksaan)
        {{-- ══════════════════════════════════════════════════════════════════════
             TAMPILAN KARTU PEMERIKSAAN (format tabel sama persis dengan create)
             ══════════════════════════════════════════════════════════════════════ --}}
        @php
            // Parse catatan: "[PMK] Lokasi: xxx | No. Seri: yyy | Jenis Kimia: zzz | ..."
            ...
            $jenisKimiaVal   = $catatanParts['Jenis Kimia']    ?? ($apar->type ?? '—');
            $beratVal        = $catatanParts['Berat']          ?? ($apar->capacity ?? '—');
            $kondisiVal      = $kartu->kesimpulan              ?? '—';
            $tglPengisianVal = $catatanParts['Tgl Pengisian']  ?? '—';
            $tglKadaluarsaVal= $catatanParts['Tgl Kadaluarsa'] ?? '—';
            ...
            <table class="w-full text-xs border-collapse" style="min-width: 900px;">
                <thead>
                    ...
                    <tr class="bg-gray-100">
                        <th rowspan="2" class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800 w-8">NO.</th>
                        <th rowspan="2" class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800">LOKASI</th>
                        <th rowspan="2" class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800 w-24">NO. SERI</th>
                        <th rowspan="2" class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800 w-28">JENIS KIMIA</th>
                        <th rowspan="2" class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800 w-20">BERAT<br>(Kg)</th>
                        <th rowspan="2" class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800 w-24">KONDISI</th>
                        <th colspan="2" class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800">TANGGAL</th>
                        <th rowspan="2" class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800 w-28">KETERANGAN</th>
                    </tr>
                    {{-- Baris header bawah (sub-header TANGGAL) --}}
                    <tr class="bg-gray-100">
                        <th class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800 w-24">PENGISIAN</th>
                        <th class="border border-gray-400 px-2 py-2 text-center font-bold text-gray-800 w-24">KADALUARSA</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td class="border border-gray-300 px-2 py-2 text-center text-gray-500">1</td>
                        <td class="border border-gray-300 px-2 py-2 text-gray-800">{{ $lokasiVal }}</td>
                        <td class="border border-gray-300 px-2 py-2 text-center font-mono text-gray-800">{{ $noSeriVal }}</td>
                        <td class="border border-gray-300 px-2 py-2 text-center text-gray-800">{{ $jenisKimiaVal }}</td>
                        <td class="border border-gray-300 px-2 py-2 text-center text-gray-800">{{ $beratVal }}</td>
                        <td class="border border-gray-300 px-2 py-2 text-center">
                            ...
                            @endphp
                            <span class="inline-flex px-2 py-0.5 text-xs font-semibold rounded-full border {{ $kondisiBadge }}">
                                {{ ucfirst($kondisiVal) }}
                            </span>
                        </td>
                        <td class="border border-gray-300 px-2 py-2 text-center text-gray-800">
                            {{ $tglPengisianVal !== '—' ? \Carbon\Carbon::parse($tglPengisianVal)->format('d/m/Y') : '—' }}
                        </td>
                        <td class="border border-gray-300 px-2 py-2 text-center text-gray-800">
                            {{ $tglKadaluarsaVal !== '—' ? \Carbon\Carbon::parse($tglKadaluarsaVal)->format('d/m/Y') : '—' }}
                        </td>
                        <td class="border border-gray-300 px-2 py-2 text-gray-700">{{ $keteranganVal }}</td>
                    </tr>
                </tbody>
            </table>
        </div>

        {{-- Bawah tabel: Tanggal & Petugas (layout sama dengan create pemeriksaan) --}}
        <div class="grid grid-cols-2 gap-8 items-end mt-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <div class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm">
                        {{ \Carbon\Carbon::parse($kartu->tgl_periksa)->format('d F Y') }}
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Petugas Pemeriksa</label>
                    <div class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm">
                        {{ $kartu->petugas }}
                    </div>
                </div>
            ...
            {{-- Kanan: Tanda Tangan (sama dengan create pemeriksaan) --}}
            <div class="text-center">
                ...
                @if($displaySignature)
                    <div class="h-20 flex items-center justify-center mb-2 -mt-14">
                ...
                <div class="border-t-2 border-gray-400 pt-2 mx-auto w-56">
                    @if($displaySignature)
                        ...
                        @if($displaySignature->nip)
                            <p class="text-xs text-gray-500">NIP: {{ $displaySignature->nip }}</p>
                        @endif
                    @else
        ...
        @else
        {{-- ══════════════════════════════════════════════════════════════════════
             TAMPILAN KARTU KENDALI (format dokumen formal sesuai standar PLN)
             ══════════════════════════════════════════════════════════════════════ --}}
            ...
            <tbody>
                @php
                    ...
                    if ($template && $template->inspection_fields) {
                        $viewRows = [];
                        foreach ($template->inspection_fields as $index => $tf) {
                            $key = $tf['key'] ?? null;
                            if ($key && in_array($key, $aparDbCols)) {
                                $dbCol = $key;
                ...
                @foreach($viewRows as $row)
                    ...
                    @endphp
                    <tr>
                        <td class="border border-gray-800 px-4 py-2 text-center">{{ $row['label'] }}</td>
