...
<head>
    <meta charset="utf-8">
    <title>Kartu Kendali Rumah Pompa - {{ $rumahPompa->barcode ?? $rumahPompa->serial_no }}</title>
...
<div class="max-w-5xl mx-auto px-4 py-6">
    <div class="sheet-a4 bg-white rounded-xl shadow-lg border p-8">
        ...
        {{-- INFO RUMAH POMPA (format tabel formal) --}}
        <table class="w-full text-sm border-collapse border border-gray-800 mb-0">
            <tr>
                ...
                <td class="border border-gray-800 px-3 py-2">: {{ $rumahPompa->location_code ?? '-' }}</td>
                <td class="border border-gray-800 px-3 py-2 font-bold w-28">TIPE</td>
                <td class="border border-gray-800 px-3 py-2">: {{ $rumahPompa->type ?? '-' }}</td>
            </tr>
            <tr>
                <td class="border border-gray-800 px-3 py-2 font-bold">ZONA</td>
                <td class="border border-gray-800 px-3 py-2" colspan="3">: {{ $rumahPompa->zone ?? '-' }}</td>
            </tr>
        </table>
            ...
            <tbody>
                @php
                    ...
                    if (!empty($kartu->inspection_data) && is_array($kartu->inspection_data)) {
                        ...
                        $inspectionRows = [
                            ['label' => 'Pompa Utama', 'value' => $kartu->pompa_utama ?? null],
                            ['label' => 'Pompa Cadangan', 'value' => $kartu->pompa_cadangan ?? null],
                            ['label' => 'Jockey Pump', 'value' => $kartu->jockey_pump ?? null],
                            ['label' => 'Panel Kontrol', 'value' => $kartu->panel_kontrol ?? null],
                            ['label' => 'Uji Fungsi', 'value' => $kartu->uji_fungsi ?? null],
                        ];
                    }
                @endphp
