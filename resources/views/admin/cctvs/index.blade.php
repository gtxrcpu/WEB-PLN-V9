<x-layouts.app :title="'Superadmin - Pilot Project CCTV'">
  <div class="max-w-7xl mx-auto px-4 py-6 space-y-6">
    ...
    <div class="space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div class="inline-flex items-center px-2 py-1 mb-2 text-xs font-semibold tracking-wide text-indigo-700 bg-indigo-100 rounded-full">Pilot Project (Superadmin Only)</div>
          <h1
            class="text-3xl font-bold tracking-tight text-slate-900">
            Manajemen CCTV Terpadu
          </h1>
          <p class="text-sm text-slate-600 mt-1">
            Pantau dan kelola indikator kualitas CCTV (Baik / Jelek) khusus untuk Superadmin
          </p>
        </div>
    ...
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 space-y-4">
      <form action="{{ route('admin.cctvs.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
        ...
        <button type="submit" class="inline-flex justify-center items-center px-4 py-2 border-2 border-indigo-600 rounded-xl text-sm font-semibold text-indigo-600 bg-white hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200">
            Filter
        </button>
        <a href="{{ route('admin.cctvs.index') }}" class="inline-flex justify-center items-center px-4 py-2 border-2 border-slate-200 rounded-xl text-sm font-semibold text-slate-600 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 transition-all duration-200">
            Reset
        </a>
      </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="flex flex-col bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-lg hover:shadow-indigo-500/20 hover:border-indigo-500/40 transition-all duration-300 group">
            ...
            <div class="relative h-48 w-full bg-slate-800 shrink-0">
                ...
                {{-- Overlay Fade (memastikan teks di atas gambar selalu terbaca jelas) --}}
                <div class="absolute inset-0 bg-gradient-to-b from-black/70 via-transparent to-black/70"></div>

                {{-- Offline State Overlay (akan menimpa gambar sepenuhnya jika kamera offline) --}}
                @if($cctv->status !== 'Baik')
                    <div class="absolute inset-0 bg-slate-900/80 flex flex-col items-center justify-center z-20">
                        <svg class="w-10 h-10 text-slate-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                        <span class="text-xs font-bold text-slate-300 tracking-widest uppercase">Koneksi Terputus</span>
                    </div>
                @endif

                {{-- Top Left: REC and Location --}}
                <div class="absolute top-3 left-3 z-30 flex flex-wrap gap-2">
                    ...
                    <div class="flex items-center px-2 py-1 bg-black bg-opacity-60 border border-slate-600 rounded">
                        <span class="text-[11px] font-mono font-medium text-white tracking-widest uppercase shadow-sm">
                            CAM_{{ str_pad($cctv->id, 3, '0', STR_PAD_LEFT) }}_{{ $cctv->location_code }}
                        </span>
                    </div>
                </div>
                
                {{-- Top Right: Status Switcher --}}
                <div class="absolute top-3 right-3 z-30 cursor-pointer hover:scale-105 transition-transform" onclick="toggleStatus({{ $cctv->id }}, '{{ $cctv->status }}')" title="Ubah Status">
                    @if($cctv->status === 'Baik')
            ...
            <div class="p-5 flex flex-col flex-grow bg-slate-900 relative z-10 border-t border-slate-800">
                ...
                <div class="flex items-center gap-2 pt-4 border-t border-slate-800 mt-auto">
                    <a href="{{ route('admin.cctvs.edit', $cctv) }}" class="flex-1 inline-flex justify-center items-center px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white text-sm font-semibold rounded-lg transition-colors border border-slate-700">
                    ...
                    <form action="{{ route('admin.cctvs.destroy', $cctv) }}" method="POST" onsubmit="return confirm('Hapus CCTV? Data tidak bisa di-restore.')" class="flex-none">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="inline-flex justify-center items-center px-3 py-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 hover:text-rose-400 text-sm font-semibold rounded-lg transition-colors border border-rose-500/20" title="Hapus CCTV">
        ...
        <div class="col-span-full bg-slate-50 rounded-2xl p-8 text-center border-2 border-dashed border-slate-200">
            ...
            <h3 class="mt-2 text-sm font-medium text-slate-900">Belum ada data CCTV</h3>
            <p class="mt-1 text-sm text-slate-500">Mulai integrasikan CCTV pertama Anda pada halaman Superadmin.</p>
            <div class="mt-6">
                <a href="{{ route('admin.cctvs.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-xl text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
  ...
  <script>
    // Live Timestamp Updater for CCTV Cards
    setInterval(() => {
        const now = new Date();
    ...
    async function toggleStatus(id, currentStatus) {
        ...
        try {
            const response = await fetch(`/admin/cctvs/${id}/status`, {
                method: 'POST',
                headers: {
            ...
            if(response.ok) {
                // Reload on success since we used simple Blade structure to keep logic synced
