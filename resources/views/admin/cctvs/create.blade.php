<x-layouts.app :title="'Admin - Tambah CCTV'">
<div class="max-w-4xl mx-auto px-4 py-6 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-600 mt-1">Lengkapi form di bawah untuk registrasi kamera pemantau baru.</p>
        </div>
        <div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('admin.cctvs.store') }}" method="POST" class="p-6 sm:p-8 space-y-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <p class="mt-1.5 text-xs text-slate-500 italic">Gunakan format URL streaming yang didukung (M3U8 untuk HLS).</p>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="floor_plan_id" id="floor_plan_id"
                            ...
                            @foreach($floorPlans as $plan)
                                <option value="{{ $plan->id }}" {{ old('floor_plan_id') == $plan->id ? 'selected' : '' }}>
                                    {{ $plan->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="floor_plan_x" id="floor_plan_x" value="{{ old('floor_plan_x') }}"
                            class="block w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors sm:text-sm p-3 border"
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="floor_plan_y" id="floor_plan_y" value="{{ old('floor_plan_y') }}"
                            class="block w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors sm:text-sm p-3 border"
                ...
                <div class="pt-2">
                    ...
                    <div class="flex items-center gap-6">
                        <label class="relative flex items-center gap-3 cursor-pointer group">
                            <input type="radio" name="status" value="Baik" {{ old('status', 'Baik') == 'Baik' ? 'checked' : '' }}
                        ...
                        <label class="relative flex items-center gap-3 cursor-pointer group">
                            <input type="radio" name="status" value="Jelek" {{ old('status') == 'Jelek' ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="block w-full rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 transition-colors sm:text-sm p-3 border"
                        placeholder="Contoh: Lensa perlu dibersihkan pada jadwal maintenance berikutnya">{{ old('notes') }}</textarea>
