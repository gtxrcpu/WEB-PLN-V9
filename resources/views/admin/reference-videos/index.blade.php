<x-layouts.app :title="'Admin - Video Referensi'">
  {{-- Header Section --}}
  <section class="mb-6 p-6 sm:p-8 shadow-lg rounded-xl bg-white">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-sm text-gray-600 ml-[52px]">Kelola video panduan dan tutorial untuk pengguna</p>
      </div>

      <a href="{{ route('admin.reference-videos.create') }}"
    ...
    @if(session('success'))
      <div
        class="mb-6 rounded-xl bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 p-4 shadow-sm animate-fade-in-up">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
      ...
      <div class="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-4 ring-1 ring-emerald-100">
        <div class="flex items-center gap-3 mb-2">
          ...
          <div>
            <p class="text-xs text-emerald-600 font-medium">Published</p>
            <p class="text-2xl font-bold text-emerald-900">{{ $videos->total() }}</p>
          </div>
        </div>
      </div>

      <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-4 ring-1 ring-orange-100">
        <div class="flex items-center gap-3 mb-2">
          <div
            class="w-10 h-10 rounded-lg bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center shadow-md">
            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div>
            <p class="text-xs text-orange-600 font-medium">Latest</p>
            <p class="text-sm font-bold text-orange-900">{{ $videos->first()?->created_at->format('d M') ?? '-' }}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        @foreach($videos as $video)
          <div
            ...
            <div class="relative aspect-video overflow-hidden bg-slate-100">
              @if($video->thumbnail_url)
                <img src="{{ $video->thumbnail_url }}"
                  ...
                  alt="{{ $video->title }}">
              @else
                <div class="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-blue-100 to-cyan-100">
              ...
              @if($video->unit_id)
                <div
                  class="absolute top-3 left-3 p-0 px-3 py-1.5 bg-gradient-to-r from-blue-600 to-cyan-600 text-white text-xs font-bold rounded-lg shadow-lg">
                  {{ $video->unit->code }}
                </div>
              @else
                <div
            ...
            {{-- Content --}}
            <div class="p-5">
              <h3 class="font-bold text-gray-900 text-base mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors">
                {{ $video->title }}
              </h3>
              ...
              <div class="flex gap-2">
                <a href="{{ route('reference-videos.show', $video) }}" target="_blank"
                  class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-600 text-white text-sm font-semibold rounded-lg hover:from-blue-700 hover:to-cyan-700 transition-all shadow-md hover:shadow-lg">
                  <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M8 5v14l11-7z" />
                  </svg>
                  Play
                </a>
                <a href="{{ route('admin.reference-videos.edit', $video) }}"
                  class="inline-flex items-center justify-center px-4 py-2 bg-slate-100 text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-200 transition-colors">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="bg-white rounded-2xl p-16 text-center shadow-lg ring-1 ring-slate-200">
        <div class="max-w-md mx-auto">
          ...
          <p class="text-gray-600 mb-8">
            Mulai bangun library video referensi dengan upload video panduan pertama Anda
          </p>

          <a href="{{ route('admin.reference-videos.create') }}"
            class="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white text-base font-bold rounded-xl hover:from-blue-700 hover:to-cyan-700 transition-all shadow-xl hover:shadow-2xl hover:scale-105">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
            </svg>
            <span>Upload Video Pertama</span>
          </a>
        </div>
      </div>
    </section>
