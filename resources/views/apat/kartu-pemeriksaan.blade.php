...
<head>
    <meta charset="utf-8">
    <title>Kartu Pemeriksaan APAT — {{ $apat->serial_no }}</title>
...
<div class="no-print bg-white border-b shadow-sm sticky top-0 z-10">
    <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
        <div>
            <h1 class="text-lg font-bold text-gray-900">Kartu Pemeriksaan APAT</h1>
            <p class="text-sm text-gray-500">{{ $apat->serial_no }}
                @if($apat->lokasi) — {{ $apat->lokasi }} @endif
            </p>
        </div>
...
<div class="max-w-5xl mx-auto px-4 py-6">
    <div class="sheet-a4 bg-white rounded-xl shadow-lg border p-8">
            ...
            <table class="w-full text-sm">
                <tr>
                    <td rowspan="{{ max(1, count($template->header_fields ?? [])) }}"
                        ...
                        <div class="font-bold text-2xl">KARTU PEMERIKSAAN APAT</div>
        ...
        <div class="flex items-start justify-between mb-6 pb-4 border-b-2 border-gray-200">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">KARTU PEMERIKSAAN APAT</h2>
        ...
        {{-- FORM --}}
        <form method="POST" action="{{ route('apat.kartu-pemeriksaan.store') }}" id="pemeriksaanForm">
            @csrf
                            ...
                            <tr class="border-t border-gray-300">
                                ...
                                <td class="px-1 py-1 border-r border-gray-300">
                                    <input type="text"
                                           ...
                                           placeholder="Lokasi APAT"
                                           class="w-full px-2 py-1.5 text-sm border border-gray-200 rounded focus:ring-2 focus:ring-amber-400 focus:border-amber-400 bg-white placeholder-gray-300">
                                </td>
                                <td class="px-1 py-1 border-r border-gray-300">
                                    <input type="text"
                                           name="rows[{{ $i }}][no_seri]"
                                           ...
                                           placeholder="No. Seri"
                                           class="w-full px-2 py-1.5 text-sm border border-gray-200 rounded focus:ring-2 focus:ring-amber-400 focus:border-amber-400 bg-white placeholder-gray-300 font-mono">
                                </td>
                                <td class="px-1 py-1 border-r border-gray-300">
                                    <select name="rows[{{ $i }}][kondisi]"
                                            class="w-full px-2 py-1.5 text-sm border border-gray-200 rounded focus:ring-2 focus:ring-amber-400 focus:border-amber-400 bg-white">
                                        <option value="">—</option>
            ...
            <div class="no-print mt-8 pt-6 border-t border-gray-200 flex items-center justify-between gap-3">
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit" id="submitBtn"
                            class="px-6 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 text-sm font-semibold shadow-md transition-colors">
...
(function () {
    ...
    if (form && btn) {
        form.addEventListener('submit', function (e) {
            ...
            btn.classList.replace('bg-amber-600', 'bg-gray-400');
