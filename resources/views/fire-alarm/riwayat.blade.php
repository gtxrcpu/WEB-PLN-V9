<x-layouts.app :title="'Riwayat Kartu Kendali — ' . $fireAlarm->barcode">
  <div class="mb-6">
    ...
    <p class="text-sm text-gray-600 mt-1">{{ $fireAlarm->barcode ?? $fireAlarm->serial_no }} -
      {{ $fireAlarm->location_code }}</p>
  </div>
  ...
  <div class="bg-white rounded-xl shadow-lg ring-1 ring-slate-200 p-6 mb-6">
    ...
    <form method="GET" action="{{ route('fire-alarm.riwayat', $fireAlarm->id) }}"
