{{-- resources/views/p3k/list-by-jenis.blade.php --}}
<x-layouts.app :title="'Manajemen P3K - ' . ucfirst($jenis)">
    <div class="max-w-7xl mx-auto px-4 py-6 space-y-6">

            @php
                $jenisConfig = match($jenis) {
                    'pemeriksaan' => ['prefix' => 'P3K-PMKS', 'label' => 'Pemeriksaan', 'btn' => 'from-emerald-600 to-green-600 shadow-emerald-500/30'],
                    'pemakaian'   => ['prefix' => 'P3K-PMK',  'label' => 'Pemakaian',   'btn' => 'from-blue-600 to-indigo-600 shadow-blue-500/30'],
                    default       => ['prefix' => 'P3K-STK',  'label' => 'Stock',        'btn' => 'from-purple-600 to-pink-600 shadow-purple-500/30'],
                };
                $totalP3k   = $p3ks->total();
                $statusBaik = $p3ks->filter(fn($p) => in_array(strtolower($p->status ?? ''), ['baik','lengkap']))->count();
                $statusRusak = $p3ks->where('status','rusak')->count();
                $totalKartu = $stats['total_kartu'];
            @endphp
        ...
        <div class="mb-4">
            <a href="{{ route('p3k.pilih-jenis') }}"
                ...
                <span>Kembali ke Pilih Jenis</span>
            </a>
        </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1
                        ...
                        Manajemen P3K — {{ ucfirst($jenis) }}
                    </h1>
                    <p class="text-sm text-slate-600 mt-1">
                        Format kode: <code class="font-mono font-semibold">{{ $jenisConfig['prefix'] }}-UNIT-NNN</code>
                    </p>
                </div>

                @hasanyrole('superadmin|leader|petugas')
                <div class="flex items-center gap-3">
                    <a href="{{ route('p3k.create', ['jenis' => $jenis]) }}"
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div
                    ...
                    <div class="relative">
                        ...
                        <p class="text-white/80 text-sm font-medium">Total Kartu {{ ucfirst($jenis) }}</p>
                        <p class="text-3xl font-bold text-white mt-1">{{ $totalKartu }}</p>
                    </div>
                </div>
        ...
        @if($p3ks->count())
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                @foreach($p3ks as $p3k)
                    @php
                        ...
                        $latestNomorKartu = match($jenis) {
                            'pemeriksaan' => $p3k->kartuPemeriksaan()->latest()->value('nomor_kartu'),
                            'pemakaian'   => $p3k->kartuPemakaian()->latest()->value('nomor_kartu'),
                            'stock'       => $p3k->kartuStock()->latest()->value('nomor_kartu'),
                            default       => null,
                        };

                        $statusConfig = match (true) {
                            in_array($statusLower, ['baik','lengkap']) => [
                    ...
                    <div data-item data-serial="{{ $p3k->serial_no }}"
                        ...
                        <div class="p-6">
                            ...
                            <div class="grid grid-cols-2 gap-3 mb-5">
                                ...
                                <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                    ...
                                    <p class="text-sm font-semibold text-slate-900 truncate">{{ $p3k->unit->name ?? '—' }}</p>
                                </div>
                            </div>
                            ...
                            <div class="relative mb-5">
                                ...
                                <div
                                    ...
                                    {{-- Download QR Button --}}
                                    <button onclick="downloadQR('{{ $p3k->qr_url }}', 'QR_P3K_{{ $kodePendek }}')"
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <a href="{{ route('p3k.kartu.create', ['jenis' => $jenis, 'p3k_id' => $p3k->id]) }}"
                                    class="group/btn relative overflow-hidden inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-to-r {{ $jenisConfig['btn'] }} text-white text-sm font-semibold hover:opacity-90 shadow-lg hover:shadow-xl transition-all duration-300">
                                ...
                                <a href="{{ route('p3k.riwayat', ['p3k' => $p3k, 'jenis' => $jenis]) }}"
                                    ...
                                    @php
                                        // kartuCount already computed above
                                    @endphp
                                    @if($kartuCount > 0)
                                        <span class="ml-1 px-2 py-0.5 bg-white/20 rounded-full text-xs font-bold">{{ $kartuCount }}</span>
                                    @endif
            ...
            {{-- Pagination --}}
            <div class="mt-4">{{ $p3ks->appends(['unit_id' => $unitId])->links() }}</div>
            ...
            <div
                ...
                <div class="relative">
                    ...
                    <p class="text-slate-600 text-sm mb-1">
                        Mulai dengan menambahkan P3K pertama
                    </p>
                    <p class="text-xs text-slate-500 mb-6">
                        Setelah menambahkan P3K, Anda dapat membuat Kartu Kendali untuk setiap unit
                    </p>
